...
function Posts() {
    ...
    const { posts, isLoading, error } = GetAllPosts()
    const { User } = UserInfo()
    ...
    return (
        <div className="w-full lg:max-w-[800px] mx-auto h-screen flex flex-col mt-9">

            {/* imagelider */}
            {posts?.length > 0 && <div className={`${isopenModal ? "opacity-100" : "opacity-0"} bg-black transition-all duration-300 ease-in-out relative`} onClick={closemodel}>
                {isopenModal && (
            ...
            <div className="w-full flex flex-col ">
                <button className="text-2xl font-bold outline-none border-none  text-end ml-2 cursor-pointer  p-3 mt-5 mb-2" onClick={() => {
                    if (!User) {
                        navigate('/login')
                        return
                    } else {
                    ...
                    <Plus role="button" className="w-10 h-10 bg-blue-200 text-blue-600 rounded-full " size={25} />
                </button>
                <ul className="mt-0 w-full flex  items-center flex-col gap-4">
                    {error ? (
                        <div>
